...
using Nop.Plugin.Widgets.BlogCategory.Models.Authentication;
    ...
    public class TokenController : Controller
    {
        ...
        public async Task<IActionResult> Create([FromBody] TokenRequest model)
        {
            ...
            if (model.Guest)
            {
                ...
                if (!await _customerService.IsInCustomerRoleAsync(newCustomer, BlogCategoryDefaults.ApiRoleSystemName))
                {
                    var apiRole = await _customerService.GetCustomerRoleBySystemNameAsync(BlogCategoryDefaults.ApiRoleSystemName);
                    if (apiRole == null)
                        throw new InvalidOperationException($"'{BlogCategoryDefaults.ApiRoleSystemName}' role could not be loaded");
        ...
        {
            return BlogCategoryDefaults.DefaultAccessTokenExpirationInDays;
        ...
        private TokenResponse GenerateToken(Customer customer)
        {
            ...
            var signingCredentials = new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(BlogCategoryDefaults.SecurityKey)), SecurityAlgorithms.HmacSha256);
